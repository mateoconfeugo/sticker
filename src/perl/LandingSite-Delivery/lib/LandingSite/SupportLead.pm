package LandingSite::SupportLead;
use Moose;
...
my @fields = (qw|event_time email full_name offer phone adnetwork adgroup listing profile campaign market_vector landing_site user_agent postal_code|);

sub create {
  my ($self, $args) = @_;
  my $lead = $args->{lead};
  my $sql = q|INSERT INTO lead_log (event_time, email, full_name, offer, phone, adnetwork, adgroup, listing, profile, campaign, market_vector, landing_site, user_agent, postal_code) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)|;

package LandingSite::Delivery::Controller::SupportLead;
use Moose;
...
sub support_lead: Local : ActionClass('REST') {}
...
sub  support_lead_POST : Path : Args(0) {
   my ($self, $c) = @_;
   $DB::single=1;
   my $lead = $c->req->data;
   my $model = $c->model('SupportLead');
   my $errors = {};
...
=cut
	   ...
	   service => 'leads',
	   state   => 'okay',
	   tags => ["support_group_leads"],
	   metric => 1,
	   description => 'SupportLead form correctly filled out'});
...
sub support_lead_GET : Path : Args(1) {
   my ($self, $c, $lead_id) = @_;
   $DB::single=1;
   my $data = $c->model('SupportLead')->get_lead({id=>$lead_id});
...
sub lead_PUT {
  ...
  my $response = $c->model('SupportLead')->create_update({lead=>$lead});
...
sub lead_DELETE {
   my ($self, $c, $lead_id) = @_;
   $DB::single=1;
   my $data = $c->model('SupportLead')->delete_lead({id=>$lead_id});

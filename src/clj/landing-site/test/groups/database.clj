...
(def service (pallet.configure/compute-service :qa-cloud))
...
(defdb db (mysql {:db (:db-name  db-settings) :user (:db-username db-settings) :password (:db-password db-settings) :host db-ip}))

(ns models.user
  (:require [clojure.java.jdbc :only[with-server with-connection with-open]:as sql]
            [expectations]
            [clojure.core :exclude[values]]
            ;;        [management.models.user :only(dashboard-config)]
            [management.models.user]
            [management.database :only[drop-database create-database create-management-database  table-builder]]
...
(def test-profile (dissoc complete-test-profile :user :id :status))
...
(def snippet-functions (map :display-with (-> active-profile :user :dashboard :display-tuple)))
(def gather-functions (map :gather-with (-> active-profile :user :dashboard :display-tuple)))
(def display-order (map :order (-> active-profile :user  :dashboard :display-tuple)))
